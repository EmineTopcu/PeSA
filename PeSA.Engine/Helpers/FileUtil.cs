    ...
    public class FileUtil
    {
            ...
            Dictionary<int, Dictionary<char, double>> WeightColumns, Dictionary<int, Dictionary<char, double>> ScaledColumns)
        {
            ...
            foreach (int pos in ScaledColumns.Keys)
            {
                ...
                worksheet.Cells[startrow, colind, rowind - 1, colind + colInc].Style.HorizontalAlignment = ExcelHorizontalAlignment.Center;
                colind += colInc + 1;
            }
            if (rowind < lastrow)
                rowind = lastrow;
